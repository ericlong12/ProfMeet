...
# Acknowledgements:
  ...
  Frances Belleza: Created and validated Office Hours, Time Slots, Courses pages, implemented form validation and UI alignment, GUI for Search and Edit Office Hours page  
...
**Add Time Slots:** Lets advisors set specific available times for advising appointments.  
...
**Search and Edit Office Hours:** Merges the previous search and edit pages into a single interface to search, edit, or delete appointments using a case-insensitive student name search.  
**View Time Slots:** Opens a table that displays all stored time slots sorted by starting time.  
**View Semester Office Hours & Courses:** Displays all saved office hour and course entries in tables.  
...
4. **Editing/Deleting Appointments:** The "Search and Edit Office Hours" page allows users to find appointments by student name (or view all) and make updates or deletions.

5. **Time Slot Validation:** The app checks to make sure the "To Hour" is later than the "From Hour". If not, it shows an error message and blocks the entry.

6. **Table Sorting:** Saved time slots are displayed in a table and are automatically sorted in ascending order by the start time (e.g., 8:00 AM before 10:30 AM). Office hour appointments are sorted descending by date and time.

# Polymorphism Usage

Polymorphism is implemented in the project using the `Page` interface:

- Parent Interface: `Page` – defines the method `Scene getScene(Stage stage)`
- Concrete Class: `SearchAndEditOfficeHoursPage` implements `Page` and provides its own version of `getScene(Stage)`
- Allows navigation between different UI pages using a common structure.